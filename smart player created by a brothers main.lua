-- Smart Player Extension for Jieshuo
-- Updated: Share opens directly on top of player without going to launcher
-- UI Language: English
...
-- ویڈیو ڈسپلے رینڈرنگ کے لیے امپورٹ شامل کیے گئے ہیں
...
-- Dynamic custom speeds cache tracking
local currentCustomSpeedSet = nil
-- Default layout scanner configuration settings
local showWhatsAppMediaToggle = "on"

-- Search, Sort & Browse Global Engine States
local currentSortMethod = "A-Z"
local currentSearchQuery = ""
local currentBrowseMode = "folders" -- "folders" or "all_files"
...
-- Native Volume Boost Applier (With optional Silent flag)
...
local function showNotification(title)
    pcall(function()
                ...
                onReceive = function(context, intent)
                    if intent.getAction() == "com.smartplayer.TOGGLE" then
                        if player.isPlaying() then
                            player.pause()
                            showToast("Paused from Notification")
                        else
                            player.start()
                            applyPlaybackSpeed()
                            showToast("Playing from Notification")
                        end
        ...
        builder.setContentTitle("Smart Player")
...
----------------------------------------------------------------------
-- جی شو بیک جیسچر سپورٹ کے ساتھ سیف یو آئی تھریڈ لانچر
----------------------------------------------------------------------
...
-- High Performance Recursive Scanner Engine
...
showAudioSettingsMenu = function()
    ...
    builder.setItems(items, function(dialog, which)
        if which == 0 then
            local durItems = {"5 Seconds", "10 Seconds", "20 Seconds", "30 Seconds", "60 Seconds"}
...
-- Playback Speed Menu (Dynamic List View based on context)
showPlaybackSpeedMenu = function(parentMenu)
    local baseSpeeds = {0.5, 1.0, 1.5, 2.0}
    local items = {"0.5x", "1.0x (Normal)", "1.5x", "2.0x"}
    
    -- Check if the current speed is a custom user-defined value
    ...
    -- If custom speed exists, insert it dynamically in the list for selection
    ...
    -- "Set Custom Speed" text input button ONLY appended inside Master Settings
    ...
    speedBuilder.setItems(items, function(d, w)
        ...
        if parentMenu == "settings" and idx == #items then
            -- Open custom safe edit dialogue box
            Handler(Looper.getMainLooper()).post(Runnable({
                run = function()
                    ...
                    customBuilder.setPositiveButton("Set", function()
                        local val = tonumber(inputField.getText().toString())
            ...
            -- Plain selection work for both menus
        ...
        run = function()
            ...
            builder.setPositiveButton("Set Timer", DialogInterface.OnClickListener({
                onClick = function(dialog, which)
                    local h = tonumber(etHours.getText().toString()) or 0
                    local m = tonumber(etMinutes.getText().toString()) or 0
                    local s = tonumber(etSeconds.getText().toString()) or 0
                        ...
                        sleepRunnable = Runnable({
                            run = function()
                                pcall(function()
                                    ...
                                    showToast("Sleep timer triggered. Player paused and minimized.")
                                end)
                            end
                        })
...
renderMediaList = function(currentPath, mediaType)
    ...
    -- Simplified Search Controller Button (Only inside folder browsing)
                ...
                Handler(Looper.getMainLooper()).post(Runnable({
                    run = function()
                        ...
                        searchDialog.setPositiveButton("Search", function()
                            currentSearchQuery = inputField.getText().toString()
...
playMedia = function(filePath)
    ...
    local success, err = pcall(function()
        ...
        player.start()
        applyPlaybackSpeed()
        showToast("Playing: " .. File(filePath).getName())
...
showPlayerControls = function()
    ...
    Handler(Looper.getMainLooper()).post(Runnable({
        run = function()
            ...
            -- یہاں ویڈیو رینڈر کرنے کے لیے SurfaceView سیٹ کیا گیا ہے
            if currentSavedMediaType == "video" then
                local surfaceView = SurfaceView(context)
                -- ویڈیو ڈسپلے کے لیے 550 پکسلز کی ہائٹ دی گئی ہے
                            ...
                            sleepRunnable = Runnable({
                                run = function()
                                    pcall(function()
                                        ...
                                        cancelNotification()
                                        if controlsDialog then controlsDialog.dismiss() controlsDialog = nil end
                                        showToast("Sleep timer triggered.")
                                    end)
                                end
                            })
                            sleepHandler.postDelayed(sleepRunnable, sleepDurationMs)
            ...
            btnPlayPause.setOnClickListener(View.OnClickListener({
                onClick = function(v)
                    if player.isPlaying() then
                        player.pause() saveState() showToast("Paused")
            ...
            btnMinimize.setOnClickListener(View.OnClickListener({
                onClick = function(v)
                    ...
                    if backgroundPlay == "on" then
                        showToast("Minimized to Background")
                        showNotification(File(currentFilePath).getName())
                    else
            ...
            btnExit.setOnClickListener(View.OnClickListener({
                onClick = function(v)
                    ...
                    currentFilePath = ""
                    lastPlayedPosition = 0
                    currentSavedFolder = "" -- Reset currentSavedFolder for clean boot on re-entry
                    currentSavedMediaType = "audio"
                    _G.smart_player_minimized = false
                    saveState()
                    cancelNotification()
                    if controlsDialog then controlsDialog.dismiss() controlsDialog = nil end
                    showToast("Exited completely.")
                end
            }))
            ...
            -- پلے کنٹرولز ڈائیلاگ پر کسٹم بیک کی (بیک جیسچر) مینیجمنٹ
            import "android.view.KeyEvent"
...
showMoreOptions = function()
    ...
    builder.setItems(options, function(dialog, which)
        ...
        elseif which == 1 then
            -- ====== بہتر اور آٹومیٹڈ شیئر لاجک ======
            saveState()
            
            -- پچھلا ایکٹو پیکیج نیم لیں (جہاں صارف ایکسٹینشن چلانے سے پہلے موجود تھا)
            ...
            -- اگر پیکیج نیم خالی ہو تو محفوظ رہنے کے لیے ڈیفالٹ لاؤنچر سیٹ کریں
            ...
            -- مینیو اور پلیئر ونڈو کو عارضی طور پر کلوز کریں تاکہ اینڈرائیڈ کی سسٹم شیئر شیٹ بلاک نہ ہو اور فوراً ٹاپ پر دکھے
            if dialog then dialog.dismiss() end
            if controlsDialog then
                controlsDialog.dismiss()
                controlsDialog = nil
            end

            -- نیٹِو شیئر شیٹ انٹینٹ فائر کریں
            pcall(function()
                local context = service
                local shareUri = nil
                pcall(function()
            ...
            -- مانیٹرنگ لوپ (Handler): صارف کے بیک کرنے پر پلیئر کنٹرولز کو خود بخود واپس لانے کے لیے
            ...
            monitorRunnable = Runnable({
                run = function()
                    ...
                    -- اگر صارف واپس بیک کر کے ہوم اسکرین, لاؤنچر، یا پچھلی ایپ پر آ گیا ہے تو پلیئر مینیو دوبارہ لانچ کریں
                    ...
                    elseif loopCount < 120 then
                        -- اگر صارف ابھی تک شیئر مینیو یا شیئرنگ ونڈو میں ہے، تو ہر 1 سیکنڈ بعد دوبارہ چیک کریں (میکسمم 2 منٹ)
...
----------------------------------------------------------------------
-- 4. انیشلائزیشن اور پلے بیک مینیجمنٹ لاجک (بیک گراؤنڈ اسکیننگ اپڈیٹ)
----------------------------------------------------------------------
loadState()
_G.smart_player_minimized = false

-- اگر کوئی فائل پہلے سے سلیکٹڈ یا ٹریکڈ ہے، تو پلے کنٹرولز کا لے آؤٹ فوری دکھائیں اور لسٹ بیک گراؤنڈ میں لوڈ کریں
if currentFilePath and currentFilePath ~= "" and File(currentFilePath).exists() then
    showPlayerControls()
    applyVolumeBoost(true) -- Pass true to make it silent on extension launch
    applyPlaybackSpeed()
    
    -- لسٹ بیک گراؤنڈ میں خاموشی سے لوڈ ہو رہی ہے تاکہ Next/Previous بٹن فوری کام کریں
    Handler(Looper.getMainLooper()).post(Runnable({
        run = function()
            pcall(function()
                local rawFiles = {}
    ...
    -- اگر کوئی فائل ٹریکڈ نہیں ہے تو فولڈر لسٹ یا مین مینو دکھائیں
    if currentSavedFolder and currentSavedFolder ~= "" and File(currentSavedFolder).exists() then
        renderMediaList(currentSavedFolder, currentSavedMediaType)
        applyVolumeBoost(true) -- Pass true to make it silent on extension launch
        applyPlaybackSpeed()
    else
