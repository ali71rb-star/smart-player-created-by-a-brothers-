...
local function matchesFormat(name, mediaType)
    ...
    if mediaType == "audio" then
        return lower:find("%.mp3$") or lower:find("%.m4a$") or lower:find("%.wav$") or lower:find("%.ogg$") or lower:find("%.amr$") or lower:find("%.opus$") or lower:find("%.aac$")
...
showWhatsAppMenu = function()
    local appItems = {"WhatsApp", "WhatsApp Business"}
    local builder = AlertDialog.Builder(service)
    builder.setTitle("WhatsApp Media Center")
    builder.setItems(appItems, function(dialog, which)
        local isBusinessApp = (which == 1)
        local baseDir
        if isBusinessApp then
            baseDir = "/storage/emulated/0/Android/media/com.whatsapp.w4b/WhatsApp Business/Media/"
            if not File(baseDir .. "WhatsApp Business Video").exists() then
                baseDir = "/storage/emulated/0/WhatsApp Business/Media/"
            end
        else
            baseDir = "/storage/emulated/0/Android/media/com.whatsapp/WhatsApp/Media/"
            if not File(baseDir .. "WhatsApp Video").exists() then
                baseDir = "/storage/emulated/0/WhatsApp/Media/"
            end
        end
        local prefix = isBusinessApp and "WhatsApp Business" or "WhatsApp"
        local subItems = {prefix .. " Audio", prefix .. " Voice Notes", prefix .. " Video", prefix .. " Statuses"}
        local subBuilder = AlertDialog.Builder(service)
        subBuilder.setTitle(prefix)
        subBuilder.setItems(subItems, function(subDialog, subWhich)
            local selectedPath = ""
            local mediaType = "audio"
            if subWhich == 0 then
                selectedPath = baseDir .. prefix .. " Audio"
            elseif subWhich == 1 then
                selectedPath = baseDir .. prefix .. " Voice Notes"
            elseif subWhich == 2 then
                selectedPath = baseDir .. prefix .. " Video"
                mediaType = "video"
            elseif subWhich == 3 then
        ...
        subBuilder.setNegativeButton("Back", function() showWhatsAppMenu() end)
        showDialogSafe(subBuilder, function() showWhatsAppMenu() end)
    end)
